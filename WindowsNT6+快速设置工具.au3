...
#PRE_Res_Field=作者博客|http://www.iczyt.cc
#PRE_Antidecompile=y
...
Global Const $UerHome = "http://www.iczyt.cc"
...
If @OSVersion = "WIN_81" Or @OSVersion = "WIN_10" Or @OSVersion = "WIN_10" Then
...
$kmsvlbtn = GUICtrlCreateButton("KMSVLALL", 285, 286, 55, 25)
GUICtrlSetTip(-1, '传说中的KMS VL ALL' & @LF & '激活工具..', '说明', 1)
	...
	For $objChassis In $colChassis
		For $strChassisType In $objChassis.ChassisTypes
			...
			If $strChassisType = 8 Or $strChassisType = 9 Or $strChassisType = 10 Then
				If $Biosinfo <> '' Then
					If StringInStr($Biosinfo, '51nb') Then
...
Func KMSVLALL()
	...
	PreFiles()
	RunWait(@TempDir & '\KMS_VL_ALL\1-KMS_VL_ALL.cmd')
	FileDelete(@TempDir & '\KMS_VL_ALL.exe')
	DirRemove(@TempDir & '\KMS_VL_ALL')
	GUICtrlSetState($kmsvlbtn, $GUI_ENABLE)
EndFunc   ;==>KMSVLALL

Func PreFiles()
	If Not FileExists(@TempDir & '\KMS_VL_ALL') Then
		FileInstall('.\file\KMS_VL_ALL.exe', @TempDir & '\', 1)
		RunWait(@TempDir & '\KMS_VL_ALL.exe -o "' & @TempDir & '" -r -y', @TempDir, @SW_HIDE)
	EndIf
EndFunc   ;==>PreFiles
...
Func ChangeTermPortNum()
	...
	If $NewPortNum <> $oldPortNum Then
		...
		RegWrite('HKEY_LOCAL_MACHINE' & $OSFlag & '\SYSTEM\CurrentContro1Set\Control\Tenninal Server\WinStations\RDP-Tcp', 'PortNumber', 'REG_DWORD', $NewPortNum)
		If _IsFirewallOpen() Then
